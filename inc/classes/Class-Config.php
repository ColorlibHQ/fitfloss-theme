...
// Final Class
final class Fitfloss {

	// Theme Version
	private $fitfloss_version = '1.0';
	...
	private function setup() {
		...
		$enqueu          = new fitfloss_Enqueue();
		$enqueu->scripts = $this->enqueue();
		$enqueu->fitfloss_scripts_enqueue_init();
	...
	public function support() {
		// content width
		$GLOBALS['content_width'] = apply_filters( 'fitfloss_content_width', 751 );


		// text domain for translation.
		load_theme_textdomain( 'fitfloss', FITFLOSS_DIR_PATH . '/languages' );
		...
		// Site logo size
		add_image_size( 'fitfloss_site_logo_82x78', 82, 78, true );

		// Popular course thumb size
		add_image_size( 'fitfloss_order_thumb_360x300', 360, 300, true );

		// Schedule & about thumb size
		add_image_size( 'fitfloss_schedule_about_thumb_780x946', 780, 946, true );

		// Gallery thumb size
		add_image_size( 'fitfloss_gallery_thumb_360x420', 360, 420, true );

		// Latest post thumbnail size
		add_image_size( 'fitfloss_widget_post_thumb', 80, 80, true );

		// Single blog post image size
		add_image_size( 'fitfloss_single_blog_750x375', 750, 375, true );
		add_image_size( 'fitfloss_np_thumb', 60, 60, true );
		...
		register_nav_menus( array(
			'primary-menu' 		=> esc_html__( 'Primary Menu', 'fitfloss' ),
		) );
	...
	private function enqueue() {
		...
		$cssPath = FITFLOSS_DIR_CSS_URI;
		$jsPath  = FITFLOSS_DIR_JS_URI;
		...
		$scripts = array(
			'style'   => array(
				...
				array(
					...
					'dependency' => array(),
					'version'    => $this->fitfloss_version,
				),
				array(
					'handler' => 'fitfloss-style',
	...
	private function google_font() {
		...
		if ( 'off' !== _x( 'on', 'Google font: on or off', 'fitfloss' ) ) {
			
			$font_families = array(
				'Anton|Oswald:300,400,500,600',
			);
	...
	private function customizer_init(){
		...
		// Instantiate fitfloss theme customizer
		$fitfloss_theme_customizer = new fitfloss_theme_customizer();
	}
	...
	public function epsilon_quickie(){
			...
			'links' => array(
				array(
					'link_to'   => 'fitfloss_options_panel',
	...
	public function set_elementor_flag(){
		update_option( 'fitfloss_had_elementor', 'no' );
	}
	// Elementor dsiable default style
	public function elementor_desiable_default_style(){

		$nonce = $_POST['nonce'];
		if ( ! wp_verify_nonce( $nonce, 'fitfloss-elementor-notice-nonce' ) ) {
			return;
		}
		$reply = $_POST['reply'];
		if ( ! empty( $reply ) ) {
			...
			update_option( 'fitfloss_had_elementor', 'yes' );
		}
		die();
	...
	public function enqueue_elementor_theme_default_style(){
		...
		if ( $disabled_color_schemes === 'yes' && $disabled_typography_schemes === 'yes' ) {
			wp_enqueue_style( 'fitfloss-elementor-default-style',  FITFLOSS_DIR_CSS_URI. 'elementor-default-element-style.css', array(), $this->fitfloss_version );
		}
	}
	// Enqueue elementor notice scripts
	public function enqueue_elementor_notice_script(){

		$had_elementor = get_option( 'fitfloss_had_elementor' );

		if( $had_elementor == 'no' && self::check_elementor_preview_page() ){
			wp_enqueue_script( 'fitfloss-elementor-notice', FITFLOSS_DIR_JS_URI.'fitfloss-elementor-notice.js', array('jquery'), '1.0', true );
			wp_localize_script(
				'fitfloss-elementor-notice',
				'fitflossElementorNotice',
				array(
					'ajaxurl' => admin_url( 'admin-ajax.php' ),
					'nonce'   => wp_create_nonce( 'fitfloss-elementor-notice-nonce' ),
				)
			);
		}
