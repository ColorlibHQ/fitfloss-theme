...
function fitfloss_post_cats() {
	...
	if ( $cats ) {
		...
		$categories .= '<ul class="nav"><li><span><i class="fa fm fa-th-list"></i>' . esc_html( 'Catagory :', 'fitfloss' ) . '</span></li>';
...
// Blog categories
function fitfloss_featured_post_cat(){
...
// social media
if ( ! function_exists( 'fitfloss_social' ) ) {
	function fitfloss_social( $args = array() ) {
		...
		$url = fitfloss_opt( 'fitfloss_social_url' );
...
// header cart count
function fitfloss_cart_count( $class = '' ) {

	?>
	<div class="header-wrapicon2 <?php echo esc_attr( $class ); ?>">
		<img src="<?php echo esc_url( FITFLOSS_DIR_ASSETS_URI ); ?>img/icons/icon-header-02.png" class="header-icon1 js-show-header-dropdown" alt="<?php esc_attr_e( 'ICON', 'fitfloss' ); ?>">
...
function fitfloss_contact7_form_content( $template, $prop ) {
...
add_filter( 'wpcf7_default_template', 'fitfloss_contact7_form_content', 10, 2 );