...
add_action( 'wp_enqueue_scripts', 'fitfloss_sl_enqueue_scripts' );
function fitfloss_sl_enqueue_scripts() {

	wp_enqueue_script( 'fitfloss-post-likes', FITFLOSS_DIR_JS_URI . 'post-likes.js', array( 'jquery' ), '0.5', false );

	wp_localize_script( 'fitfloss-post-likes', 'simpleLikes', array(
		'ajaxurl' => admin_url( 'admin-ajax.php' ),
		'unlike'  => esc_html__( 'Unlike', 'fitfloss' )
	) );
}
 ...
 * @since    0.5
 */
add_action( 'wp_ajax_nopriv_fitfloss_process_simple_like', 'fitfloss_process_simple_like' );
add_action( 'wp_ajax_fitfloss_process_simple_like', 'fitfloss_process_simple_like' );
function fitfloss_process_simple_like() {
	// Fitness
	$nonce = isset( $_REQUEST['nonce'] ) ? sanitize_text_field( $_REQUEST['nonce'] ) : 0;
	if ( !wp_verify_nonce( $nonce, 'simple-likes-nonce' ) ) {
		exit( esc_html__( 'Not permitted', 'fitfloss' ) );
	}
...
function get_simple_likes_button( $post_id, $is_comment = NULL ) {
	...
	if ( already_liked( $post_id, $is_comment ) ) {
		...
		$title = esc_html__( 'Unlike', 'fitfloss' );
		$icon = $icon_full;
	} else {
		$class = '';
		$title = esc_html__( 'Like', 'fitfloss' );
		$icon = $icon_empty;
	}
...
function get_like_count( $like_count ) {
	$like_text = esc_html__( '', 'fitfloss' );
	if ( is_numeric( $like_count ) && $like_count > 1 ) { 
		$number = sl_format_count( $like_count ) . esc_html__( ' Likes', 'fitfloss' );
	} else {
		$number = sl_format_count( $like_count ) . esc_html__( ' Like', 'fitfloss' );
	}
...
// User Profile List
add_action( 'show_user_profile', 'fitfloss_show_user_likes' );
add_action( 'edit_user_profile', 'fitfloss_show_user_likes' );
function fitfloss_show_user_likes( $user ) { ?>
	<table class="form-table">
		<tr>
			<th><label for="user_likes"><?php _e( 'You Like:', 'fitfloss' ); ?></label></th>
			<td>
			<?php
			...
			<?php else : ?>
			<p><?php esc_html__( 'You do not like anything yet.', 'fitfloss' ); ?></p>
			<?php 
			endif; 
			wp_reset_postdata(); 
			?>
