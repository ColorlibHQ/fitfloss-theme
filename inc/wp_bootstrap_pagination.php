...
function fitfloss_pagination( $args = array() ) {
    
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => esc_html__( '&laquo;', 'fitfloss' ),
        'next_string'     => esc_html__( '&raquo;', 'fitfloss' ),
        'before_output'   => '<div class="fitfloss-pagination-area col-12"><nav><ul class="pagination">',
        'after_output'    => '</ul></nav></div>'
    );
    
    $args = wp_parse_args( 
        $args, 
        apply_filters( 'fitfloss_pagination_defaults', $defaults )
    );
